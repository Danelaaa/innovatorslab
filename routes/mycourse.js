...
router.get("/", verifyToken, async (req, res) => {
    ...
    try {
        ...
        // 2. Fetch steps with progress and presentation/quiz info
        const [steps] = await connection.execute(`
            SELECT cs.step_number, cs.presentation_id, cs.quiz_id,
                   p.title AS presentation_title, p.file_path AS presentation_file,
            ...
            WHERE cs.course_id = ?
            GROUP BY cs.step_number, cs.presentation_id, cs.quiz_id, p.title, p.file_path, q.title, up.step_number
        ...
        const certificate = certResult.length > 0 ? `/certificates/certificate_${userId}_${courseId}.pdf` : null;
        ...
        presentationsByLang.forEach(({ step_number, language, file_path }) => {
            ...
            presentationFiles[step_number].push({ language, file_path });
        });
