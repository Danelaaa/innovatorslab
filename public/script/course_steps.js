...
function populateCourseSteps(courseSteps) {
    ...
    courseSteps.forEach(step => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="edit-btn" onclick="editCourseStep(${step.id}, ${step.step_number}, '${step.presentation_title}', '${step.quiz_title}')">✏️ Edit</button>
                <button class="delete-btn" onclick="deleteCourseStep(${step.id})">🗑 Delete</button>
            </td>
        `;
    });
}
...
// Add Course Step
document.getElementById('courseStepForm').addEventListener('submit', async function (e) {
    e.preventDefault();
const course_id = 1;
    ...
    try {
        const response = await fetch('/api/course-steps', {
            ...
            body: JSON.stringify({course_id, step_number, presentation_id, quiz_id })
        });
...
function editCourseStep(id, stepNumber, presentationTitle, quizTitle) {
    document.getElementById('stepNumber').value = stepNumber;
    document.getElementById('presentationSelect').value = presentationTitle;
    document.getElementById('quizSelect').value = quizTitle;

    editStepId = id;

    document.getElementById('addStepBtn').style.display = 'none';
    document.getElementById('updateStepBtn').style.display = 'inline-block';
}

// Update Course Step
document.getElementById('updateStepBtn').addEventListener('click', async function () {
    ...
    try {
        const response = await fetch(`/api/course-steps/${editStepId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ step_number, presentation_id, quiz_id })
        });
        ...
        document.getElementById('addStepBtn').style.display = 'inline-block';
        document.getElementById('updateStepBtn').style.display = 'none';
        editStepId = null;
    } catch (error) {
        console.error('Error updating course step:', error);
    }
});
